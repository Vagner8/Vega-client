...
import { ControlFormcontrolsIndicators, Fractal, FractalForm, Fractals, FractalsDto } from '@types';
  ...
  const formRecord = new FormRecord(
    fractal.controls.reduce((acc: Record<string, FormRecord>, controlDto) => {
      acc[controlDto.indicator] = new FormRecord(
        Object.values(ControlFormcontrolsIndicators).reduce((acc: Record<string, FormControl>, field) => {
